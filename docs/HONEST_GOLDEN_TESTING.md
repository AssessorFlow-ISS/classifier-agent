...
|---|---|---|
| ~~`vendor/af_shared/` shim (4 modules) — missing `adapters.factory` + `pubsub.agent_subscriber`~~ | RESOLVED 2026-05-04 — shim extended with `adapters/{factory.py, stubs/{decision_audit_stub.py, tracing_stub.py}}` + `pubsub/agent_subscriber.py`. Real `assessorflow/shared` PAT install still preferred long-term but no longer blocking | Provision `SHARED_REPO_PAT` in repo secrets when convenient; current shim is sufficient |
| ~~CI's `pytest --cov-fail-under=0` override~~ | RESOLVED 2026-05-04 — gate restored to `--cov-fail-under=90` once shim above unblocked the 5 previously-skipped tests (`test_pubsub_adapter`, `test_api`, `test_main_adapter_wiring`, `test_main_handler`, `test_tracing_wiring`) | n/a |
| ~~Hardcoded `prompt_version=@v1` in `model_broker_http.py`~~ | RESOLVED 2026-05-05 — adapter now requires a caller-supplied `prompt_version` kwarg and raises `ValueError` if missing. Three callers (`ReactSufficiencyProber`, `TopicExtractor`, `RubricFitnessAssessor`) pass `self._prompt_version` resolved from `prompts/<template>/MANIFEST.yaml`. Langfuse traces now carry the real version (e.g. `@v6` for `react_sufficiency`). New regression test `test_invoke_raises_when_prompt_version_missing` guards against re-introduction. See `docs/PROMPT_VERSIONING.md` | n/a |
| ~~No prompt-version archival (in-place bumps lose history)~~ | RESOLVED 2026-05-05 — every template now lives in `prompts/<template>/` with per-version `vN.yaml` files plus `MANIFEST.yaml`. The loader resolves `current: vN` at load time. `react_sufficiency/` carries v3/v4/v5/v6 (with v3/v4 marked `body_preserved: false` honestly). See `docs/PROMPT_VERSIONING.md` | n/a |
| ~~No main-branch prompt-regression gate~~ | RESOLVED 2026-05-05 — `.github/workflows/prompt-regression.yml` fires post-merge on `paths: ['prompts/**']` and runs DeepEval drift + Promptfoo OWASP smoke. Pre-merge variant is residual debt (see report §B.6.7) | Future: prune the smoke set to ≤90s wall and promote to pre-merge |
